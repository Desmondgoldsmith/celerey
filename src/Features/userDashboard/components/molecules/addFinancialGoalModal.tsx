...
import { AddFinancialGoalModalProps, GoalFormData } from "../../types";
...
const AddFinancialGoalModal: React.FC<AddFinancialGoalModalProps> = ({
  ...
  const [formData, setFormData] = useState<GoalFormData>(() => ({
    ...
    targetAmount:
      (initialData?.type === "emergency"
        ? initialData?.targetValue.split(" ")[0]
        : initialData?.targetValue.toString()) || "",
    currentAmount:
      (initialData?.type === "emergency"
        ? initialData?.currentValue.split(" ")[0]
        : initialData?.currentValue.toString()) || "",
    ...
    targetDuration: initialData?.duration?.toString() || "",
  }));

  const { loading, createFinancialGoal, updateFinancialGoal, setLoading } =
    useDashboardStore();
  ...
  // Calculate start and end dates based on target duration
  const calculateDates = (targetDuration: string) => {
    if (!targetDuration || isNaN(parseInt(targetDuration)))
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (updatedFormData.durationStart && updatedFormData.durationEnd) {
        ...
        updatedFormData.targetDuration = goalDuration;
      }
  ...
  // Update start and end dates when target duration changes
  useEffect(() => {
    if (formData.targetDuration) {
      const { startDate, endDate } = calculateDates(formData.targetDuration);

      setFormData((prev) => ({
        ...prev,
        durationStart: startDate,
        durationEnd: endDate,
        goalDuration: formData.targetDuration,
        durationLeft: formData.targetDuration,
      }));
    }
